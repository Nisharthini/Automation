...
module.exports = function () {
  ...
  /*  this.When(/^I click on the "([^"]*)"$/, function(objectKey) { // VehiclesButton, InventoryLink
        return page.common.clickElement(objectKey)
    });*/

  /* this.When(/^I enter inputs for login$/, function(dataTable) { // VehiclesButton, InventoryLink
        const rows=dataTable.rows()
        for(var i=0; i<rows.length;i++){
            var inputname=[i][0]
            var inputvalue=[i][1]
            page.common.inputElement(inputname, inputvalue)
        }
        return;
    });*/
